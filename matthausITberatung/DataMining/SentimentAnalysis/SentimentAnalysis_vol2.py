...
print(df_filtered[['TopicId','Opinion','Adjustment','Sentiment']])
...
df_filtered['WeightedPolarity'] = df_filtered.apply(lambda row: weightedPolarity(row['Opinion'], row['Adjustment']), axis=1)
print(df_filtered)

topic_weightedPolaity = df_filtered.groupby('TopicId')['WeightedPolarity'].sum() / df_filtered.groupby('TopicId')['Adjustment'].sum()
df_topic_weightedPolaity = pd.DataFrame({'TopicId': topic_weightedPolaity.index, 'WeightedPolaity': topic_weightedPolaity.values})
print(df_topic_weightedPolaity)


####Sentiment for cluster
cluster_polarity = df_topic_weightedPolaity['WeightedPolaity'].mean()
